 ...
 */
export const PUBLISHED_OPERATION_NAMES: Readonly<Record<string, string>> = {
  // DNS
  ListRecords: 'list',
  CreateRecord: 'create',
  GetRecord: 'get',
...
/** The shape `assignOperationNames` needs. `ConsolidatedOperation` satisfies it. */
...
function derivedName(operationId: string): string {
 ...
 */
export function assignOperationNames(
  operations: readonly NameableOperation[],
  infer: (operation: NameableOperation) => string
): Map<string, string> {
  ...
  for (const op of operations) {
    if (names.has(op.operationId)) continue;
    const inferred = infer(op);
    const claimants = claims.get(inferred);
    if (claimants) claimants.push(op.operationId);
    else claims.set(inferred, [op.operationId]);
  }
  ...
  for (const [inferred, claimants] of claims) {
    ...
    for (const operationId of claimants) {
      ...
      if (taken.has(name)) name = operationId;
